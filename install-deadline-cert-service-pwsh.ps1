...
function Main {
    ...
    if (-not $skip_configure_aws) {
        if (-not $aws_region -or -not $aws_access_key -or -not $aws_secret_key) {
            ...
            Write-Host "cd deploy/firehawk-main/modules/terraform-aws-vpn/modules/pi-vpn-client/scripts/firehawk-auth-scripts"
            Write-Host "./sign-ssh-key --generate-aws-key"
            Write-Host
